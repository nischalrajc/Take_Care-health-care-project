    ...
    useEffect(() => {
        const fetchDoctorData = async () => {
            try {
                ...
                const monthlyDoctorsCounts = response.data.monthlyDoctorsCounts;
                const doctorCount = response.data.totalCount
                setDoctorCount(doctorCount)
                setDoctorData(prevState => ({
                    ...prevState,
                    datasets: [{
                        ...prevState.datasets[0],
                        data: monthlyDoctorsCounts,
                    }]
                }));
            } catch (error) {
                console.log(error)
                ...
                <div className='w-full px-8 mt-8'>
                    <div className="flex flex-row">
                        ...
                        <div className="w-full lg:w-1/2 px-4 mb-4 lg:mb-0">
                            <div className="bg-thirdShade rounded-lg px-4 py-4">
                                <div className="flex justify-center mb-3 font-protest"><p>Users Count</p></div>
                                {doctorData && doctorData.datasets && <BarChart chartData={doctorData} />}
                                <div className="flex justify-center mt-3">
                                    <p className="font-protest">Total Users : {doctorCount}</p>
                                </div>
                            </div>
                        </div>
                    </div>
