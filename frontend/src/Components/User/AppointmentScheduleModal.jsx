...
function AppointmentScheduleModal({ doctorId, isOpen, onclose }) {
    ...
    useEffect(() => {
        const fetchAvailableSlots = async () => {
            try {
                const response = await Axios.get(`/available_slots?doctorId=${doctorId}&date=${selectedDate}`);
                if (response.data) {
    ...
    const selectHandler = async (id, status) => {
        setSlotId(id);
    ...
    const submitHandler = async () => {
        try {
            if (paymentStatus === 'pending') {
                alert('currently unavailable server busy')
                return
            }
            if (slotId) {
                const response = await Axios.post('/checkout-session', { userId, doctorId, slotId });
                if (response.data) {
                                    ...
                                    <div className='ms-6'>
                                        {availableSlots.map((slot, index) => {
                                            ...
                                            return (
                                                <div
                                                    key={index}
                                                    onClick={Scheduled ? null : () => selectHandler(slot._id, slot.status)}
